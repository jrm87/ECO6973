...
1. **started reviewing** material from basic econometrics course
...
Small note: In the language of Mullainathan and Spiess (2017), $\hat{y}_i$ is the best linear __prediction__ [ML], while $\hat{\beta}_1$ is the best estimation of the __causal effect__ of $x$ on $y$ [CI].
...
# Simulation
set.seed(12468)
sim_df <- lapply(X = 1:1e4, FUN = function(x, size = n_s) {
...
We can estimate a regression line in .mono[R] (`lm(y ~ x, my_data)`). But where do these estimates come from?
...
$$ \hat{\beta}_1 = \dfrac{\sum_i (x_i - \overline{x})(y_i - \overline{y})}{\sum_i (x_i - \overline{x})^2} =\dfrac{\hat{Cov}(X,Y)}{\hat{V}(X)}$$
...
.mono[R] default to testing hypotheses against the value zero.
